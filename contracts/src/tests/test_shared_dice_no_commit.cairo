...
#[test]
fn test_shared_dice_no_commit__multiple_throw() {
    let setup = setup();

    let consumer = setup.consumer.contract_address;
    let entrypoint = 'shared_dice_no_commit';
    let calldata = array![];
    ...
    // provider give random
    submit_random_no_proof(setup.provider, seed, 333);
    ...
    assert(rand == 333, 'rand should be 333');
    setup.consumer.shared_dice_no_commit();
